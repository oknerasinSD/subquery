...
export async function handleEvent(event: SubstrateEvent): Promise<void> {
    ...
    //The balances.transfer event has the payload \[from, to, value\] that we can access
    ...
    //if not in DB then instantiate a new Account object using the toAddress as a unique ID
    if (!toAccount) {
